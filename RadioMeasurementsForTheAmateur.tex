...
\item remove the crystal and insert an 100$\Omega$ trim potentiometer. Adjust this to the same amplitude level as measured for the series connection. Remove the potentiometer and use an ohm meter to measure the resistance. This is your R_{S} resistance.\\ \\ \\
	R_{S} : $\rule{3cm}{0.1mm}$ $\Omega$. % this resistance can be calculated as well, I just need to derive that exact math, its somthing like (Z0/2 * dB/1-dbcal)
...
\item With these calculations done, the crystal parameters are characteriszed. The accuracy of the frequency measurment is what defines the accuracy of this method. 
